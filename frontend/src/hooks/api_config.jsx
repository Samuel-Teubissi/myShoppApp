...
export const searchArticles = async ({
  ...
  const filteredArticles = visiblesArticles.filter((art) => {
    const matchName = art.article
      .toLowerCase()
      .includes(search_article.toLowerCase())
    const matchCategory =
      search_categ === '0' || art.category === parseInt(search_categ)
...
export const getDefaultArticles = async ({ controllerLink, page = 1 }) => {
  ...
  const response = await api.get(
    `/articles/${controllerLink}/?page=${page}&controller=${controllerLink}`,
  )
  return response.data

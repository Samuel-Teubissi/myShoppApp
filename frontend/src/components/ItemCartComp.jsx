        ...
        <div className="grid grid-cols-2 grid-rows-2 md:grid-cols-[200px_repeat(4,1fr)] md:grid-rows-1 text-left justify-start gap-3 gap-x-5 md:gap-4 pb-2 px-2 dark:text-white/90 items-center md:border-none border-app-300 dark:border-app-900 border-b">
          ...
          <div>
            ...
            <div className="flex mt-2">
              <div className="flex w-full gap-2 justify-between items-center bg-gray-100/70 dark:bg-app-600/5 rounded-full">
                <button
                  onClick={decrease}
                  className="btn-modal-qty btn-trans"
                  disabled={orderQty <= 1}
                >
                  <MinusIcon className="h-5 w-5" />
                </button>
                <span className="">
                  {_.padStart(String(orderQty), 2, '0')}/
                  {_.padStart(String(item.quantity), 2, '0')}
                </span>
                <button
                  onClick={increase}
                  className="btn-modal-qty btn-trans"
                  disabled={orderQty >= item.quantity}
                >
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <span className="hidden cart--item-box bg-app-h bg-opacity-30 w-full md:block">
              {cart_total.toLocaleString('fr-FR')}
            </span>
            <span className="md:hidden">
              {cart_total.toLocaleString('fr-FR')} F
            </span>
          </div>
          <div
            className="col-span-2 md:col-span-1 flex justify-center items-center bg-red-800 md:bg-transparent lg:hover:bg-transparent hover:bg-red-700 hover:text-white transition duration-300 text-white/90 gap-4 py-2 rounded-[5px]"
            onClick={() => {
              removeFromCart(item.id_articles)
            }}
          >
            <span className="md:hidden">Effacer</span>
            <FontAwesomeIcon
              icon={faTrashCan}
              className="w-5 h-5 text-white md:text-app-h cursor-pointer hover:text-app btn-trans transform hover:scale-110"
              title="Supprimer"
            />
          </div>
        </div>

...
const ModalAddComp = ({ isOpen, onClose }) => {
  ...
  const schemaFormAdd = Yup.object().shape({
    article: Yup.string().required("Nom de l'article requis"),
    // .min(9, 'Entrez un numéro valide de 9 chiffres')
    // .matches(/^[0-9]+$/, 'Format de numéro incorrect'),
    price: Yup.string().required("Prix de l'article requis"),
    // .min(1, "Au moins 2 chiffres")
    // .positive("Entrez une valeur correcte"),
    quantity: Yup.string()
      // .positive("Entrez une valeur correcte")
      .required('Quantité requise'),
    userfile: Yup.mixed().test('required', 'Une image est requise', (value) => {
    ...
    //         }),//fileType
    category: Yup.string().required("Catégorie l'article est requis"),
    ...
    onSuccess: async (response) => {
      if (response.status === 'success') {
        ...
        if (!fileRef.current.files[0]) {
          setAddFormError((prev) => ({
            ...prev,
            userfile: 'Un fichier visuel est requis',
          }))
        }
      }
    },

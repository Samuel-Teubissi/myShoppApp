...
const RegisterPage = () => {
  ...
  const { Register, isRegisterLoad, registerErrors, setRegisterErrors } =
    useAuth()
  ...
  const schema = Yup.object().shape({
    ...
    confirm_password: Yup.string()
      ...
      .oneOf(
        ...
        'La valeur doit être la même que celle du champ Mot de passe',
      ),
  })
  const {
    register,
    ...
    <>
      ...
      <div className="ms_Main main-about flex justify-center items-center">
        ...
        <div className="text-center overflow-hidden relative md:w-8/12 h-full backdrop-blur md:backdrop-blur-none md:h-min flex flex-col justify-center items-center rounded-xl md:border dark:border-none md:border-app-200 sm:shadow-md shadow-none md:my-4 pt-10 md:pt-0">
          ...
          <form
            className="w-full h-full text-gray-600 py-10 sm:bg-app-100/5 sm:dark:bg-app-600/10 backdrop-blur flex justify-center flex-wrap dark:text-dark-app-100 overflow-hidden"
            onSubmit={submitRegister}
          >
            <div className="hidden md:flex gap-4 items-center border-2 border-transparent bg-app-900/80 text-white dark:bg-app-900/80 px-6 py-10 mb-10 text-left font-medium sm:w-full sm:text-center sm:border-t-app-700">
              <div>
                <InfoIcon className="w-14 h-14" />
              </div>
              <div className="text-left flex flex-col">
                <strong className="uppercase">
                  Rejoignez notre grande communauté de vendeurs.
                </strong>{' '}
                Devenez un business man prospère dès aujourd'hui !
              </div>
            </div>
            ...
            <div className="w-full flex justify-center mt-4">
              <button
                type="submit"
                className={`text-white flex items-center justify-center rounded py-5 md:h-[45px] w-[90%] md:w-40 text-lg bg-app-600 hover:bg-app transition duration-300 ease-in-out ${
                  isRegisterLoad && ' text-center text-white cursor-not-allowed'
                }`}
                disabled={isRegisterLoad}
              >
                {!isRegisterLoad ? (
                  'Inscription'
                ) : (
            ...
            <div className="w-full mt-4">
              ...
              <Link to="/login" className="text-app hover:underline">
                Connectez-vous !
              </Link>
            </div>
          </form>
